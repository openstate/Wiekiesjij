...
class RefUploadedFile(UploadedFile):
    ...
    # string format
    files_reg = re.compile(r'^(?P<ctype>[^:]+):(?P<charset>[^:]+):(?P<hash>.+)$')

    def __init__(self, hash, file, content_type, size, charset):
        # name, content_type, size, charset
        self.hash = hash
        super(self.__class__, self).__init__(file, None, content_type, size, charset)
    ...
    def __unicode__(self):
        ...
        return u"%s:%s:%s" % (self.content_type, self.charset, self.hash)
    ...
    def restore(cls, value):
        ...
        if pt is not None:
            (ctype, charset, hash) = pt.groups()
            path = os.path.join(cls.tempdir(), "%s.upload" % hash)
            if os.path.isfile(path):
                file = open(path, "r")
                return RefUploadedFile(hash, file, ctype, os.path.getsize(path), charset)
    ...
    def handle_files(self, reqfiles):
        ...
        for (name, files) in reqfiles.iterlists():
            ...
            for file in files:
                ...
                ls.append(RefUploadedFile(hash, dstfile, file.content_type, file.size, file.charset))
            # end for
            ret.setlist(name, ls)
        # end for
